...
/**
 * POST /api/admin/delete-website-language.php
 *
 * Removes a website language from the registry — only one that is off, is
 * not the default and holds no words anywhere. Every translation table
 * refuses to lose its language (ON DELETE RESTRICT), so a language with
 * translations can only ever be switched off; this never takes words with
 * it.
 *
...
try {
    SiteLanguages::remove($code);
} catch (\InvalidArgumentException $e) {
    website_language_redirect($e->getMessage());
} catch (\Throwable $e) {
    error_log('[api/admin/delete-website-language.php] ' . $e->getMessage());
