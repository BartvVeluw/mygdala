...
| **Mediabibliotheek** | Herbruikbaar publiek sitebeeld: identiteit, naam en alt-tekst (één opslag), virtuele mappen, raster en lijst, hergebruik, gebruiksoverzicht, veilig verwijderen, en de ene kiezer waarmee elk scherm kiest of uploadt | `Service\Media\*` (o.a. `MediaService`, `MediaFolderService`), `MediaRepository`, `MediaFolderRepository`, `admin/media.php`, `admin/_media_picker.php`, `api/admin/*media*.php` — zie `MEDIA.md`; leesbare publieke adressen: voorstel in `docs/media/PUBLIC-URLS.md` |
...
| **Formulieren** | Formulierdefinities, velden en hun breedte, het uploadveld, publieke verwerking, meldingen, bewaarde inzendingen met hun bestanden, het voorbeeld in de editor | `Service\Forms\*` (o.a. `FormFieldWidth`, `FormFileTypes`, `FormUploadInspector`), `FormRepository`, `FormSubmissionRepository`, `ContactAttachmentStorage`, `partials/form.php`, `api/form-submit.php`, `api/admin/form-submission-attachment.php`, `admin/forms.php`, `admin/form.php`, `admin/form-preview.php`, `admin/form-submissions.php` — zie `FORMS.md` |
| **Contact/aanvragen** | Historische offerteaanvragen met bijlagen, herroepingsverzoeken, rate limiting | `ContactRequestRepository`, `ContactAttachmentStorage` (ook de opslag van het uploadveld), `ContactRateLimiter`, `TurnstileVerifier`, `admin/contact-requests.php` — nieuwe inzendingen lopen sinds Core Forms via `FORMS.md` |
